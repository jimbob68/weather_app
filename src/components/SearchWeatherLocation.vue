...
export default {
  ...
  methods: {
      submit() {
            fetch("https://api.tomtom.com/search/2/structuredGeocode.json?countryCode=" + this.selectedCountry.cca3 + "&municipalitySubdivision=" + this.searchTerm + "&key=" + process.env.VUE_APP_tomTomKey )
            .then(res => res.json())
            .then(results => {
